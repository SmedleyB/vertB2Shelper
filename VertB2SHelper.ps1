...
function Get-ResContent {
    ...
    @(
        ...
        '# Display device'
        '1'
        '# Backglass position: X, Y'
        $Layout.BackglassX
        $Layout.BackglassY
        '# FullDMD: width, height'
        $dmdWidth
        $dmdHeight
        '# FullDMD position: X, Y (relative to backglass)'
        $dmdX
        $dmdY
        '# Y-flip'
        '0'
...
$numOffset = Add-NumberControl -Top $y -Minimum 0 -Maximum 7680 -Value 0 -Left 150
$y += 30
...
$numDmdW = Add-NumberControl -Top $y -Minimum 100 -Maximum 3840 -Value 1080 -Left 150
$y += 30
[void](Add-Label 'Element Gap (px):' $y)
$numGap = Add-NumberControl -Top $y -Minimum 0 -Maximum 500 -Value 20 -Left 150
